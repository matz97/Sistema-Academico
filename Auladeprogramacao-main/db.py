...
def _ensure_sqlite_schema(connection):
    ...
    if "perfil" not in usuario_columns:
        connection.execute(
            "ALTER TABLE usuarios ADD COLUMN perfil TEXT NOT NULL DEFAULT 'admin'"
        )
    if "aluno_id" not in usuario_columns:
        connection.execute("ALTER TABLE usuarios ADD COLUMN aluno_id INTEGER NULL")
    if "professor_id" not in usuario_columns:
        connection.execute("ALTER TABLE usuarios ADD COLUMN professor_id INTEGER NULL")
    ...
    if "nota1" not in columns:
        connection.execute("ALTER TABLE matriculas ADD COLUMN nota1 REAL NULL")
    if "nota2" not in columns:
        connection.execute("ALTER TABLE matriculas ADD COLUMN nota2 REAL NULL")
    ...
    connection.execute(
        """
        UPDATE matriculas
           SET nota1 = 8.5, nota2 = 9.0
         WHERE nota1 IS NULL
           AND aluno_id = (SELECT id FROM alunos WHERE matricula = ?)
...
def _ensure_mysql_schema(connection):
    ...
    if "aluno_id" not in columns:
        cursor.execute("ALTER TABLE usuarios ADD COLUMN aluno_id INT NULL")
    if "professor_id" not in columns:
        cursor.execute("ALTER TABLE usuarios ADD COLUMN professor_id INT NULL")
